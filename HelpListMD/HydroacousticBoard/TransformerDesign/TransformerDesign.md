...
2. **设计方案**    
   ...
   - **漆包线直径**：
      参考趋肤深度$\epsilon=\frac{6.62}{\sqrt{f}}K(cm)$，这里使用$Cu$作为导线，$K=1$，最高频率为$120kHz$，则可计算出直径为 $0.038cm$。  

   - **匝数**：
      $N_{p}=\frac{V_{p}}{A_{c}B_{AC}fK_{f}}$，pwm波经过了低通滤波，可以近似看作正弦波，则$K_{f}=4.44$，$B_{AC}$为交流磁通密度，$V_{p}$为一次电压，算出一次侧绕组匝数为$N_{p33k}=27$，$N_{p120k}=7$。
   - **匝比**：  
   ...
   - **电流密度**  
      ...
      暂定电压$V=48V$，一次侧绕组$P_{in}=62.5W$，效率为$\eta=0.8$，漆包线直径$0.019cm$，则二次侧绕组功率为$P_{out}=50W$，则一次侧电流$I_{in}=1.302A$，二次侧电流$I_{out}=0.104A$。假定$J=2A/mm^2$，则反推出一次侧漆包线直径$d_{in,min}=0.91mm$,二次侧漆包线直径$d_{out,min}=0.26mm$。  

   - **磁心结构**
      一次侧的漆包线直径$0.91mm$，按方形绕制面积计算，截面积为$0.83mm^2$，27匝，二次侧漆包线直径$0.26mm$，按方形占位面积计算，截面积为$0.068mm^2$，共$270$匝，则总窗口面积所需为$0.83 \times 27+0.068\times 270=41.0mm^2$。  
      
      ![alt text](image-5.png)
      ![alt text](image-6.png)

      其中可绕线圈的窗口面积计算公式为$(A-B)*2*E$，根据表格计算出所需要的型号为$EE28$，其窗口面积最小为$109.12mm^2$。

   - **励磁电感**
      由电感定义：$V=L\frac{dI}{dt}$，其中$L$为电感值，$I$为电流。  
      从而$L_{m}=\frac{V}{\omega \Delta I_{m}}$。 
      由$\omega = 2\pi f$，则$L_{m}=\frac{V}{2 \pi f \Delta I_{m}}$。由此也可以得出，电压固定，频率一定时，控制电流大小的就是电感值 。
      参考Ti的变压器设计手册纹波电流$\Delta I_{m}$取满载电流的$20\%-40\%$ 
      假定一次侧满载电流$I_{in}=1.3021A$，励磁电流取$30\%$，则一次侧励磁电感$L_{m,in,33k}=\frac{48V} {2 \pi \Delta I_{m,in} \times 33kHz}=0.59mH$,$L_{m,in,120k}=\frac{48V} {2 \pi \Delta I_{m,in} \times 120kHz}=0.16mH$。
   ...
   - **气隙控制**
      ...
      假定$\mu_{0}=4\pi \times 10^{-7}H/m$，$A_{e}=82.9mm^2$，$N=27$，$L_{m,in}=0.59mH$，则$l_{g,33k}\approx 0.13mm$,$l_{g,120k}\approx 0.47mm$。
