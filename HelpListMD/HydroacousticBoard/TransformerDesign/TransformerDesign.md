...
2. **设计方案**    
   ...
   - **匝数**：
      ...
      则磁通密度$\Delta B = \frac{V \times t_{on}}{NA_{c}}$, $A_{c}$为磁心的磁路有效面积。 
      为了避免饱和,要求 $\Delta B < 2B_{max}$ ,                                                                                                                                                                             
      从而$N > \frac{V \times t_{on}}{2B_{max} A_{c}}$。
      由$t_{on}=\frac{D}{f}$
      从而$N>\frac{V \times D}{2B_{max} A_{c}f}$,其中$D$为占空比,$f$为最低频率,$B_{max}$为最大磁通,$A_{c}$为磁心的磁路有效面积。  
      暂定$B_{max}=0.15T$，$D_{max}=0.5$，$V_{max}=48V$，$A_{c}=31.92mm^2$，$f=33kHz$，从而计算得出$N_{min}=76$。
   
   - **匝比**：  
      暂定1:1，注意匝数比和阻抗也有关系。阻抗比=（匝数比）^2.
   ...
   - **窗口面积**
      由上面的磁心结构图可知，可用于绕制线圈的窗口面积最小为$(E_{min}-D_{max})\times F_{min}$，即$(16.0-6.05)mm\times 5.4mm \times 2=107.46mm^2$，假设均分为一次侧和二次侧，则分别$53.73mm^2$和$53.73mm^2$，一次侧的漆包线直径$0.81mm$，按最大方形绕制面积计算，可绕制$81$匝。大于上面计算出的$76$匝，虽然有点挤的样子，但是实际上二次侧线直径更小，有更多的冗余空间。
